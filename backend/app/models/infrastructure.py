...
class LocationPosition(UUIDPrimaryKeyMixin, AuditMixin, Base):
    ...
    electrical_panel: Mapped[str | None] = mapped_column(String(100))
    network_switch: Mapped[str | None] = mapped_column(String(100))
    switch_port: Mapped[str | None] = mapped_column(String(50))
    patch_panel: Mapped[str | None] = mapped_column(String(100))
    junction_box: Mapped[str | None] = mapped_column(String(100))
    mounting_details: Mapped[str | None] = mapped_column(Text)
    infrastructure_details: Mapped[dict | None] = mapped_column(JSONB)
    remarks: Mapped[str | None] = mapped_column(Text)
